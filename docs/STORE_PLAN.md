...
schema_meta     key PK, value      -- store_id, default_org, identity_armed, oidc_issuer,
                                   -- oidc_join_role, import_done, import_report, legacy_hashes
...
- **`session_epoch`** is the first thing the database buys.
  ...
  - **Every** session reader uses it: the auth gate, `/auth/me`, the
    ...
    The pwflow reader in `POST /auth/change-password` and `/skip` applies
...
- **The journeys overlay** takes the workspace over (`/workspace` on its
  ...
  moves the workspace, so the pack files are copied across.
...
The rest of the workspace (packs, snapshots, journeys, runs, `deploys.jsonl`)
can be copied live as before. This supersedes PRODUCTIZATION_PLAN Stage 4's
...
2. **Guard against a stale re-import.** When the database is created, it
   ...
   `<base>/.store-imported`, holding the `store_id` and a SHA-256 of each
   legacy file imported; the same hashes go into
   ...
   - Legacy files present, a marker present, and the database empty or
     ...
     store the files were imported into and the ways out (point
     `OBSERVOGRAM_DB` at it, `packc store restore`, or `packc store import
     --replace`).
   - Ids match, but a legacy file's hash differs from the last import or
     export: the files were edited outside the store, for instance during
     a downgrade. Refuse with the same pointer.
   - `import_done` is set but the marker is missing (a crash between commit
     and marker): rewrite the marker.
3. **While `import_done` is absent:**
   - **Strict-read the legacy files.** `users.json`, `orgs.json` and each
     `packs/index.json` are read with readers that throw on a read error,
     a parse error or the wrong shape. A throw aborts `start()`, names the
     path and imports nothing. Today's lenient readers are never used here.
   - **Apply today's refusals, with no writes.** Refuse off loopback with
     no auth, where identity means OIDC env or a users file. Refuse while
     the users file still holds the seeded default and the server is
     exposed. Refuse when `orgs.json` exists with no identity. A refused
     boot moves, seeds and imports nothing.
   ...
   - **Run the import**, in one `tx()`: it records `import_done`, the
     report and one audit row, then writes the marker.
4. **Seed and check against the store.**
   - Run `maybeSeedDefaultAdmin()`, which backs off on `identity_armed`.
     Its `OBSERVOGRAM_ADMIN_PASSWORD` rescue applies only to a row still
     marked `seeded_default` + `must_change`.
   - Then run the fail-closed bind checks: no auth off loopback, a
     still-seeded default credential, and more than one org without
     identity.
   - The recorded `oidc_issuer` must equal today's canonical issuer key.
     Otherwise the server refuses to start and names both values.
   - Zero owners in the current identity mode logs a warning naming the
     way in. The same banner shows in Settings.
5. `setWorkspaceRootResolver()`, then `loadWorkspacePacks()` per org.
...
**The CLIs never import, and never write `import_done`.** `npm run users`,
`npm run orgs` and `packc store` refuse while legacy files exist at the
resolved workspace and `import_done` is absent ("start the server once
with its environment"). Otherwise they could import with a shell's env,
without the unit's `OBSERVOGRAM_USERS_FILE` or `OBSERVOGRAM_OIDC_ISSUER`.

On a workspace with nothing to import, they create the schema and their
own rows, and print "store initialised from this shell's environment",
so `npm run users -- add` on a fresh install keeps working. They print the
database path they opened, and they refuse `:memory:`.

### What the import maps

1. **Users.** `users.json` (at `OBSERVOGRAM_USERS_FILE` when set) becomes
   `users` rows with kind `local`, session epoch 0.
...
4. **Pack index.** `packs/index.json` (per org root) becomes `packs` rows.
   ...
   is dropped only on positive evidence that the file is gone, and an
   orphan file is adopted.
5. **Backfill.** Services and environments are created from the registered
   packs by one pure, browser-safe helper, `tools/lib/service-keys.mjs`.
...
- **`users.json`** is written only when `identity_armed` is set. It holds
  the enabled local users only; OIDC rows never go in it.
...
- **`orgs.json`** is written only if the deployment had one or has more
  ...
  - When it writes `orgs.json` while the default org's root is `.`, the
    pre-store boot will move that org's data into `orgs/default/`. The
    export rewrites the default org's journey `file:` paths to match, and
    prints the CronJob change (`OBSERVOGRAM_WORKSPACE=<base>/orgs/default`).
...
- **In place, with the server stopped**, it records the hashes of what it
  wrote, so a later store boot does not refuse its own export.
- **`packc store import --replace`**, with the server stopped, re-imports
  users, orgs and memberships from edited files. It keeps the audit,
  services, environments and endpoints. Users missing from the files are
  disabled, and every changed or disabled user's epoch is bumped. It writes
  one audit row.
...
|---|---|---|---|
| Open, loopback | `OBSERVOGRAM_AUTH=off` and no `OBSERVOGRAM_API_TOKEN` | the default org | actor `local`, an owner, every route. Owner routes that create identity (a user, an owner, an org, `oidc_join_role`) also need the `X-Observogram-CSRF: 1` header, so a cross-site form cannot arm identity |
| Open, exposed | `OBSERVOGRAM_INSECURE_NO_AUTH=1` | the default org | actor `local` keeps every route that exists today. Owner routes that create or change identity answer 403: "add the first user with `npm run users -- add`, or configure OIDC" |
...
| `admin` | plus the org's name, MCP endpoints and memberships in **its own org**: add an existing user by exact login or verified email, change a role, remove. An admin cannot list the deployment's users. The org's last admin can be removed only by an owner |
...
- **The route table.** One middleware after the org middleware reads an
  ...
  - `self`: the change-password routes and "sign out my other sessions",
    acting on the caller's own row. An anonymous caller is redirected to
    `/auth/login` (or gets 401 JSON);
...
| 2 | **Identity on the store** | the boot order (§4), with its strict readers, stale-import guard and marker; the import and its report; the `migrateFlatWorkspace()` fixes; users, orgs, memberships, owners; `auth.mjs`, `tenancy.mjs`, `user-admin` and `org-admin` over repositories, with validated `--role`; `resolveSession()` and `session_epoch` (pwflow included); the issuer key, OIDC just-in-time users, `oidc_join_role`, `OBSERVOGRAM_BOOTSTRAP_ADMIN`, `npm run users -- owner`; the org context in every posture; `orgWorkspaceRoot()` over `orgs.root`; `tenancyEnabled()` retired; per-org schedule snippets; journeys loaded only from the org root; `packc store export`, `import --replace`, `purge-org` | ports `server/test-auth-local.mjs`: fixtures are written before the first open or through repositories, and file-absence checks become row checks. Ports `test-tenancy.mjs`: the `:182` assert becomes a row check, and WS3 (a one-org `orgs.json` with only a bearer) now boots token-only, so it is rewritten to two orgs and the CHANGELOG says so. Ports `test-auth-oidc.mjs`: on its empty workspace the import sets `oidc_join_role = operator`, so "API reads work with the OIDC session" holds; it adds a `OBSERVOGRAM_OIDC_JOIN_ROLE=none` user who gets the "ask an admin" 403, and the bootstrap cases. `test-smoke.mjs` keeps `AUTH=off` plus a token as token-only. Docs: README security posture, store export/import, `.env.example` (users file as import source only, tenancy no longer armed by `orgs.json`, `OBSERVOGRAM_BOOTSTRAP_ADMIN`, `OBSERVOGRAM_OIDC_JOIN_ROLE`), `deploy/k8s/README.md` and the `cronjob-journeys.yaml` tenancy comment with its `test-deploy-manifests.mjs` pin (one CronJob per org at `/workspace/<orgs.root>`), the PRODUCTIZATION_PLAN status lines |
| 3 | **Roles enforced** | the route table (class plus audit actions) and its middleware; the authz matrix; the live pack per org, with `safeMcpUrl` | README (roles); CHANGELOG (members mapped to `viewer` lose write power; the live pack moves); the PRODUCTIZATION_PLAN Stage 3 status |
| 4 | **Services, environments, MCP endpoints** | tables, role-gated CRUD API, `tools/lib/service-keys.mjs` and the backfill (§4 step 5); `packs` replaces `packs/index.json`; `pack_services` links on register; the conformance tier comes from the service record when one exists | ports `server/test-workspace.mjs` and the `index.json` assertions in `test-smoke.mjs`; the API in README |
| 5 | **Audit** | the audit rows per the route table; `GET /api/audit` (filter by actor, kind, target, time; org-scoped for admins, deployment-wide for owners) | README (audit) |
...
| Import | fixture workspaces, each asserted row by row with its report: flat stand-alone; `orgs.json`-armed with a not-yet-moved flat workspace (labels kept); a flat entry plus its `orgs/default/` twin (reported, nothing merged); an acme-only `orgs.json` plus the empty `default` artefact (dropped); OIDC with `orgs.json`; OIDC without it; a leftover `users.json` under OIDC (disabled, no owner); legacy `.tomograph/`; a corrupt `index.json`; an empty `users.json` (armed, nothing seeded); unknown and messy role strings (`owner`, `Admin`, ` Viewer `, `editor`, `admn`, null, missing) |
| Stale import | deleting the database, or pointing `OBSERVOGRAM_DB` at a new path, with legacy files and a marker present: the boot refuses and imports nothing. A database moved with its id, or restored from `packc store backup`, boots. A crash after commit repairs the marker. Export in place, then a pre-store build removes a user and changes a password, then re-upgrade: the boot refuses; after `import --replace` the removed user is disabled, their old cookie is refused and the new password works |
...
| OIDC upgrade | a pre-upgrade OIDC cookie reads `/api` on a workspace without `orgs.json`, as `operator`, and two IdP users who have never signed in do too; an `orgs.json` OIDC member keeps their role with a pre-upgrade cookie and after a fresh sign-in, with an env issuer with and without a trailing slash; a local username equal to an IdP sub does not capture the membership; an unverified email never matches `OBSERVOGRAM_BOOTSTRAP_ADMIN`; a user who signed in before it was set becomes owner at their next sign-in; once an owner exists, a further match grants nothing; each grant leaves one `owner.bootstrap` row; a changed issuer refuses to boot |
...
- **Behaviour changes for existing deployments,** named in the CHANGELOG:
  ...
  - A one-org `orgs.json` with only a bearer token now boots token-only
    instead of refusing.
