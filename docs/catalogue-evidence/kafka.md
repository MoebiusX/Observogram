...
- **Kafka:** `apache/kafka:3.9.2` per the lab's image pin (`stack/kafka/Dockerfile` `ARG KAFKA_TAG=3.9.2`, `docker-compose.yaml`) — the broker exposes no version series under the Strimzi rule set: no `app-info` family (`kafka.server:type=app-info,id=1` has no `name=` key for the rules to match) and no `3.9.2` in any broker series. The only occurrence in the exposition is `clientSoftwareVersion="3.9.2"` on `kafka_server_socket_server_metrics_connections_software{clientSoftwareName="apache-kafka-java"}` — the client library of the 6 Java connections to the broker (the lab's own tools and its controller-listener client) — and `kafka_broker_info{address="kafka:9092",id="1"}` from kafka_exporter carries no version. What the expositions do state: `jvm_runtime_info{version="21.0.10+7-LTS"}`, `jmx_exporter_build_info{version="1.6.0"}`, `kafka_exporter_build_info{version="1.10.0"}`. KRaft, one node carrying both roles (broker + controller), replication factor 1. Topics `orders` (3 partitions, ~50 msg/s from `kafka-producer-perf-test`, consumer group `orders-consumers`, auto-commit) and `payments` (3 partitions, ~10 msg/s, group `payments-consumers`).
...
| — why not TotalTimeMs | `max(kafka_network_requestmetrics_totaltimems{request="FetchConsumer",quantile="0.99"}) / 1000` | **0.102–0.103 s** on a healthy broker whose topics are **not** idle (`orders` 49/s, `payments` 9.8/s; FetchConsumer requests 57.6/s at 14:40Z). The FetchConsumer quantiles at 14:40Z, in ms — TotalTimeMs p50 20 / p75 21 / p95 101 / p98 101 / p99 102 / p999 104; RemoteTimeMs the same shape (20 / 21 / 100 / 101 / 102 / 489); LocalTimeMs 0 at every quantile; RequestQueueTimeMs p99 0; ResponseSendTimeMs p99 1. Total time is the purgatory wait for the next record: 20 ms is the inter-arrival at 50 msg/s, ~100 ms the inter-arrival at 10 msg/s (`kafka-producer-perf-test --throughput 10` paces one record per 100 ms and `fetch.min.bytes=1` completes the fetch on arrival), and the idle-partition wait is `fetch.max.wait.ms` (500 ms by default): the p999 reaches 503 ms (`max_over_time` over 6 h) in the gaps when the lab's generator restarts each 60-second producer run, while the saved exposition's p999 (15:52 local) was 105.9 ms. TotalTimeMs would breach the 50 ms objective with no broker fault; LocalTimeMs p99 stays at 0–1 ms (max over 6 h: 1 ms) |
...
4. `fetch_latency_p99` described as "server-side latency" with no choice between local and total time: `TotalTimeMs` is dominated by the long-poll wait for the next record and breaches the 50 ms objective with no fault (§1.4); the SLI reads `LocalTimeMs`. (The first measured version of this section called the lab's topics "idle" and read the ~100 ms as the idle wait; the topics carry 49 and 9.8 msg/s, the ~100 ms is the 10 msg/s topic's inter-arrival, and the idle wait is the 500 ms p999 — corrected on review.)
...
6. The remediation section (§6 before this document was renumbered, §7 now) said `automation: null` for controller churn; the pack says `automation: "manual-only"`.
...
## 2. SLI selection — why these six vital signs
...
**Rationale:** fetch latency drives consumer-side lag. A degrading fetch latency on the broker side is the upstream cause of the consumer-lag SLO miss before lag has even risen. `LocalTimeMs` rather than `TotalTimeMs` because a consumer fetch is a long poll: the broker parks the request in purgatory until the next record arrives or `fetch.max.wait.ms` (500 ms by default) elapses, and `TotalTimeMs` is that wait (`RemoteTimeMs`, §1.4). On the lab's 10 msg/s topic the TotalTimeMs p99 is 102 ms — the gap between records — and on an idle partition it is the 500 ms cap, seen as the 503 ms p999 during the producer's restart gaps; both above the 50 ms objective with no broker fault, while LocalTimeMs p99 is 0–1 ms.
