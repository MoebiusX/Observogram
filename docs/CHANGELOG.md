...
Observogram monitors the artefacts that monitor the system; step 2 of the roadmap starts acquiring the observability stack's *own* health signals through the MCP so later steps can build stack-health evidence and early warning. Every number is a point-in-time sample — signal, not verdict: nothing in this step creates a `Verified` stamp, an SLO verdict or a grade change, and on a restricted MCP tier the answer is "not attempted" with the reason, never "absent".
- **Alias table (contracts).** New data-only `tools/lib/contracts/stack-self-metrics.mjs`: `STACK_SELF_METRIC_PROBES` — 24 rows over nine families (scrape, ruler, notify, tsdb, collector, dashboards, synthetic, logs, traces), each with its signal, unit, display-only direction, the reference-pack SLI whose vocabulary it follows, ordered product aliases (`{ product, expr, requires }`) and the upstream docs the names come from — plus lookup resolvers (`probeRows`, `rowsForFamily`, `eligibleAliases`, `productPreferenceOrder`, `displayHint`, `bestOutcome`) and the allowed sets (`STACK_FAMILIES`, `STACK_UNITS`, `STACK_DIRECTIONS`, `STACK_OUTCOMES`). The table is grounded in upstream documentation and the crawled pack, not a recording (no credentials or Docker were available); a known discrepancy is documented on the row rather than fixed — the reference pack's `scrape_duration_p99` names `scrape_duration_seconds_bucket`, which Prometheus never exposes (the metric is a per-target gauge), so `scrape_duration_max` samples `max(scrape_duration_seconds)`.
...
- **Synthetic fixtures.** `tools/fixtures/mcp/synthetic/` holds hand-written samples for each new shape (an otel-mcp-server and a Prometheus-API instant vector, Alertmanager status and silences, Grafana datasources, datasource health, contact points), each carrying a `_synthetic` marker the shape check must tolerate and `tools/test-contract-shapes.mjs` asserts; they pin shape only (no `adapted/` goldens — the parsers land with the sampler). The fixtures README gains a "Recorded vs synthetic" section.
- **Suites**: new `tools/test-stack-self-metrics.mjs` (`npm run test:stack`, in `npm test` and `test:contracts`) — unique ids, allowed sets, every alias's `requires` ⊆ the metric names its `expr` reads, every `referenceSli` resolves in its reference pack, resolver purity; `npm run lint:contracts` parse-checks the three contract modules. Docs: `MCP_INTEGRATION.md` "Stack self-metrics (registry)".
