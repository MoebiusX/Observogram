...
### Diff engine: operator whitespace and `ref:` prefixes stop reading as drift
- **PromQL canon tightens binary-operator whitespace.** `tools/lib/promql-canon.mjs` now removes the spaces around the symbolic operators `+ * / % ^ == != <= >= < > =~ !~` (outside strings, parser-proven expressions only), so `a / b` ≡ `a/b` and `rate(x[5m]) > 0.5` ≡ `rate(x[5m])>0.5` — the whitespace a rule picks up on the way through a ruler or a template no longer flags the recording rule as drifted. `-` is left alone (unary/binary ambiguity, `a - -b` stays as written), keyword operators (`and`/`or`/`unless`/`bool`/`offset`/`by`/`on`/…) keep the space that bounds them, label-value strings stay byte-identical, and a space *inside* an operator (`a < = b`) is never glued into a different one. Decision-bearing differences (`>` vs `>=`, thresholds, `=~` vs `!~`) still read as drift.
- **`ref:` is authoring syntax, not behaviour.** `tools/lib/artefact-model.mjs` `behaviorFor` strips a leading `ref:` from `slo`, `sli`, `trigger`, `error_budget_policy`, and from `expr` when the whole value is a reference (`ref:slis.x`), via a pure `stripRefFields` copy of the spec; identity already stripped it, so a burn alert declared `slo: ref:x` paired with the live `slo: x` and then reported itself drifted on its own binding. The partial-evidence rule for `expr: ref:slis.x` vs a compiled expression keeps working on the stripped form.
- Both modules are vendored downstream (`docs/VENDORING.md`): additive behaviour only, no new imports, signatures unchanged — re-copy the pair together. Suite: `tools/test-promql-canon.mjs` pins every operator, the string/minus/keyword fences, the glued-operator guard, the negative cases and idempotency; `tools/test-diff.mjs` pins `ref:x` ≡ `x` for burn alerts and `error_budget_policy`, a different SLO target still not aligning, and the partial-evidence path.

### Journeys: studio grade parity, vantage-aware gate, vantage-lost records
- **One score for one comparison.** `tools/lib/journey.mjs` now attaches the requirement-chain comparison (`comparePackBranches`) to the diff as `traceabilityGraph` before grading — exactly what the studio's `/api/diff` does — so the CLI and the studio score Drift-free on the same construct instead of the CLI grading on raw diff buckets. The run record says which construct scored it (`grade.driftConstruct`: `requirement-chain` / `diff-buckets`) and carries the chain rollup (`traceability`).
- **The vantage is part of the verdict.** Run records gain `probes {attempted, succeeded, empty, failed, unsupported}` (family names), `probeErrors`, `vantage`, `toolsExposedCount`, `scrapeJobsDown` and `unhealthyRules`, read from Pack B's fetcher annotations (a file-sourced B reports empty lists, `0`, `null`, `none`). Two gate keys act on them: `failOnPartialEvidence: true` breaches when any probe family failed (`live evidence is partial: probes failed: … — verdict not trustworthy`) or the vantage is entirely lost; `maxUnhealthy: N` breaches when scrape jobs down + unhealthy rules exceed N. Empty and unsupported families never breach on their own. The markdown report prints a `Live probes` line and an `On-wire health` line.
...
- Suite: `tools/test-journey.mjs` pins both gate keys (partial / empty / restricted / lost / exact-sum), the record fields on a planted live-like Pack B, studio parity (with vs without the attached graph — the journey equals the graph construct), the vantage-lost record via a closed loopback port (offline, ~instant) including the CLI exit code, and the no-record rule for config errors.

### Live fetcher: burn-rate alerts are mapped from discovered rules, never synthesised
- **False-assurance fix.** `tools/fetch-live-pack.mjs` used to emit a two-window burn-rate alert for every inferred SLO and stamp `mcp.verified.policy.burn_rate_alerts` whenever *any* alert name came back from the MCP — a live pack could report a Verified paging policy that did not exist in production. `spec.policy.burn_rate_alerts` is now built from the alerting rules the MCP actually exposes: compiler-labelled rules (`slo`/`burn_rate`/`window_short`/`window_long`/`severity`) and `<slo>_burn_<N>x_<short>_<long>`-named rules are grouped per SLO (deduped, short window first), and each emitted entry gets an **indexed** `mcp.verified.policy.burn_rate_alerts[<i>]` stamp — the unindexed key is gone.
- **Placeholder SLO re-identification.** A discovered burn group whose SLO shares the SLI base of an inferred placeholder (`svc_checkout_availability_99` → `svc_checkout_availability_99_9`) re-identifies that SLO to the discovered id and replaces its placeholder objective/window from the rule's `slo_objective`/`slo_window` annotations (only 7d/28d/30d/90d windows). Groups naming an SLO nobody inferred, or carrying a single window (the schema requires two), are reported in `mcp.discovered.alert_rules_unmapped` instead of being padded; forecast rules (`labels.kind=forecast`) are skipped. Severities outside the `SEVn` enum are mapped from the usual Prometheus vocabulary, or from the burn factor when absent, and listed in `mcp.discovered.alert_rules_severity_inferred`.
- **Live scaffold marker.** When nothing maps, the schema-forced single placeholder is stamped `mcp.scaffold.policy.burn_rate_alerts[0]`; the adapter now treats `mcp.scaffold.` as a second scaffold prefix beside `crawler.scaffold.`, so the placeholder projects as `Scaffold` (parked by the grade) rather than `Declared` or `Verified`.
- Suite: `tools/test-fetch-live.mjs` pins the placeholder path (no alert probe, threshold-only alerts, single-window group) and a compiler-labelled mapping case (ordering, dedupe, name-pattern merge, forecast skip, re-id + objective replacement, indexed stamps, unmapped annotation, adapter Verified/Scaffold projection).

### Live fetcher: probe-outcome honesty — unsupported families, probe errors, vantage
- **"Not exposed" is no longer indistinguishable from "no answer".** A probe family whose every candidate is absent from `tools/list` was recorded as `unsupported` by the probe loop but never emitted; the studio read it as "probed, none found" and the summary warned that the family "returned empty". The pack now stamps `mcp.probesUnsupported` (a restricted MCP tier) next to `mcp.probesFailed` (a hole of unknown size), and `mcp.probeErrors.<family>` carries the last erroring candidate's message (trimmed to 200 chars) so the SRE sees *why* a probe got nothing. `fetchMcp` returns its `probeFailures` map; `buildCanonicalPack` accepts it.
- `partialLiveEvidence` (vendored, zero-import) gains additive keys only: `unsupported`, `errors`, and `vantage` (`full` / `partial` / `restricted` / `lost` / `none`); `partial` keeps meaning outright failures. `GET /api/live-status` returns `probesFailed` and `probesUnsupported`; the MCP badge goes to its error state for failed probe families too and names a restricted tier in its title; the draft summary lists unsupported families as "— not exposed by this MCP" (with the probe error inline for failed ones) and warns once about the restricted tier instead of per-family "returned empty"; the partial-evidence banners append the not-exposed families.
- Suite: `tools/test-fetch-live.mjs` pins the unsupported/failed split, the per-family error mapping (last candidate wins, trimmed, never attributed to an unsupported family or an unmatched key); `tools/test-diagnostic-grade.mjs` pins every vantage; `server/test-smoke.mjs` plants a minimal live pack when none exists and pins the new live-status fields.

### Live fetcher: on-wire liveness — scrape target health, rule evaluation state
- **False-assurance fix (continued).** A scrape job NAME used to be enough to stamp `mcp.verified.telemetry.scrape` and `mcp.verified.pipelines.exporters.metrics`, and a recording/alerting rule's presence was enough to verify it — even when the MCP was reporting, in the same payload, that every target of the job was `down` or that the rule was failing to evaluate. The `scrape_configs` probe now adapts to per-job target lists (`[{job, targets:[{instance, health, lastScrape, lastError}]}]`, tolerant of otel-mcp-server's `metrics_targets` and Prometheus `/api/v1/targets`); only jobs with at least one target `up` (or of unknown health) land in `mcp.discovered.scrape_jobs` and attest scraping, jobs whose every target is down go to `mcp.discovered.scrape_jobs_down`, and every target's on-wire health is kept in `mcp.observed.scrape_targets` (JSON, 200 entries, `lastError` trimmed to 200 chars). When no job is up the metrics exporter falls back to its scaffold marker.
- **Rule health.** The `recording_rules` / `alert_rules` adapters carry `health`, `lastError`, `lastEvaluation`, `evaluationTime` (and `state`, `activeAt` for alerting rules) when the ruler reports them; the fetcher strips them before `spec.queries.recording_rules` (the schema forbids extra keys) and writes `mcp.observed.recording_rules` / `mcp.observed.alert_rules` plus `mcp.discovered.recording_rules_unhealthy` / `alert_rules_unhealthy`. Recording rules are now stamped **per index** (`mcp.verified.queries.recording_rules[<i>]` — the symbol the adapter actually reads; they used to project `Declared` despite the group stamp) and only when healthy or health-less; a burn-rate group fed by an unhealthy alerting rule still maps but earns no indexed stamp. Absent health is never treated as unhealthy; legacy job-name `string[]` probe results still count as health-less scrape evidence.
...
- Adapted fixtures regenerated (`tools/fixtures/mcp/adapted/scrape_configs.json` is now per-job target health; `recording_rules.json` / `alert_rules.json` carry the observation fields). Suite: `tools/test-fetch-live.mjs` pins the excluded down job, the down list, the observed targets (trimmed error, null health), the all-down / legacy / adapter-shape cases, per-index rule stamps with the failing rule `Declared`, the observed rule arrays, and the unhealthy-fed burn group.

### Live fetcher: placeholders are Scaffold, MTTD is no longer derived from anomaly thresholds
- **False-assurance fix (continued).** Live packs no longer fabricate burn-rate alerts or MTTD targets, and every schema-forced placeholder the fetcher invents is now stamped `mcp.scaffold.<symbol>` — never `mcp.verified.<symbol>` — so the adapter projects it as `Scaffold` and the grade parks it: the hard-coded `spec.otel` block (`system_health` answering used to verify it), collector receivers/processors and the logs/traces exporters, the fallback `logs-elastic` backend, the `platform-overview` dashboard stub, the SEV1 → Teams route, the baselines, and the per-service / `platform_availability` SLI+SLO guesses. `pipelines.exporters.metrics` is `Verified` only when scrape targets or a metric inventory evidenced it; the fallback `metrics-prom` backend only when a `*_build_info` capture (prometheus/victoriametrics/mimir) exists, `traces-jaeger` only when the topology names jaeger or `traces_services` answered. SLIs inferred from real recorded rules stay `Verified`; an SLO re-identified by a discovered burn-rate group is now `Verified` too (the old-id scaffold marker is dropped).
- **MTTD derivation removed.** `spec.baselines.mttd_target_p50` was the smallest `anomalies_baselines` `thresholdMs` (e.g. `90ms`) stamped `Verified` — a latency-anomaly threshold is not a time-to-detect target. Baselines are now the platform default for the declared criticality with `measurement_source: platform-default`; `mcp.baselinesComputed` still counts the anomaly baselines returned (the tool answered), which is not an MTTD measurement.
- Suite: `tools/test-fetch-live.mjs` pins every scaffold key and its adapter projection (route, dashboard stub, logs/traces exporters, baselines, otel, SLI/SLO guesses, fallback backends), the build_info-attested `metrics-prom` path, the evidenced metrics exporter, and the re-identified SLO stamp.
