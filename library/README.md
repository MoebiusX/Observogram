...
moment it is ticked) or with another custom one is a usage error naming both, and so is
an SLO id two SLIs would share (`broker_availability` overridden to 0.9999 and a custom
`broker_availability_99` at 0.99 both make `broker_availability_99_99`);
